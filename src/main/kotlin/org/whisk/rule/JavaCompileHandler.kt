...
//
        ...
        val deps = rule.srcs.map { it.file }
        val exportedDeps = rule.exported_deps.map { it.file }
...
//        val params = srcs.map { it.toString() }
        javaCompiler.compile(rule.srcs.map { it.file }, dependencies, classesDir.toFile())
